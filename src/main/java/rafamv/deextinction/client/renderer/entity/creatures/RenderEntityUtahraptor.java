package rafamv.deextinction.client.renderer.entity.creatures;

import net.minecraft.client.Minecraft;
import net.minecraft.client.renderer.GlStateManager;
import net.minecraft.client.renderer.entity.RenderLiving;
import net.minecraft.entity.Entity;
import net.minecraft.entity.EntityLivingBase;
import net.minecraft.util.ResourceLocation;
import net.minecraftforge.fml.relauncher.Side;
import net.minecraftforge.fml.relauncher.SideOnly;
import rafamv.deextinction.client.renderer.model.creatures.ModelUtahraptor;
import rafamv.deextinction.common.entity.creature.EntityUtahraptor;

@SideOnly(Side.CLIENT)
public class RenderEntityUtahraptor extends RenderLiving
{

	public RenderEntityUtahraptor()
	{
		super(Minecraft.getMinecraft().getRenderManager(), null, 1.0F);
		this.mainModel = new ModelUtahraptor();
	}

	@Override
	protected void preRenderCallback(EntityLivingBase entityLivingBase, float partialTickTime)
	{
		this.applyModelCharacteristics((EntityUtahraptor) entityLivingBase);
	}

	public void applyModelCharacteristics(EntityUtahraptor creature)
	{
		((ModelUtahraptor) this.mainModel).setChildCharacteristics(creature.getGrowthStage());
		float scale = creature.getModelScale();
		this.shadowSize = scale;
		GlStateManager.scale(scale, scale, scale);
	}

	@Override
	protected ResourceLocation getEntityTexture(Entity entity)
	{
		return this.getEntityTexture((EntityUtahraptor) entity);
	}

	protected ResourceLocation getEntityTexture(EntityUtahraptor creature)
	{
		return creature.getTexture();
	}
}
