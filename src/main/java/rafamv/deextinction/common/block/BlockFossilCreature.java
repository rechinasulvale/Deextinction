...
public class BlockFossilCreature extends BlockOriented
{

	public BlockFossilCreature(String mainBlock)
	{
		super(Material.rock);
		this.setUnlocalizedName("block_" + mainBlock + "_fossil_creature");
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ)
	{
		...
		else if (!playerIn.isSneaking())
		{
			...
			if (stack != null && stack.getItem() instanceof ItemPlasterBurlapStrips)
			{
				...
				if (period != null)
				{
					float creatureNoise = DEFossilHelper.getNoise2DForFossils(pos.getX(), pos.getZ());
					Creature creature = DEFossilHelper.getCreature(period, creatureNoise);
					...
					if (tileEntity instanceof TilePlasterJacket)
					{
						...
						if (creature != null)
							plasterJacket.setFossilName(creature.getName());
						else
