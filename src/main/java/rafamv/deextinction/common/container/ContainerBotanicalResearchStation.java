...
public class ContainerBotanicalResearchStation extends Container
{
	private TileBotanicalResearchStation research_station;
	private int prev_research_progress;
	private int prev_creating_progress;
	private int prev_nutrients;

	public ContainerBotanicalResearchStation(InventoryPlayer playerInventory, TileBotanicalResearchStation research_station)
	{
		...
		this.addSlotToContainer(new SlotSwitchable(this.research_station, 0, 8, 28)
		{
			...
			{
				return super.isItemValid(stack) && ContainerBotanicalResearchStation.canPlaceFossil(stack);
			}
		});
		...
		this.addSlotToContainer(new SlotSwitchable(this.research_station, 1, 8, 28)
		{
			...
			public boolean isItemValid(ItemStack stack)
			{
				return super.isItemValid(stack) && ContainerBotanicalResearchStation.canPlaceNutrient(stack);
			}
		});
		...
		this.addSlotToContainer(new SlotSwitchable(this.research_station, 2, 25, 54)
		{
			...
			public boolean isItemValid(ItemStack stack)
			{
				return super.isItemValid(stack) && ContainerBotanicalResearchStation.canPlaceRequiredItem(stack);
			}
		});
	...
	public static boolean canPlaceFossil(ItemStack itemStack)
	{
		if (itemStack != null)
		{
			if (itemStack.getItem() instanceof ItemFoliageItem)
				return true;

			for (Foliage foliage : DEDatabaseRegistry.LIST_ALL_FOLIAGE.values())
			{
				Set<ItemStack> fossils = foliage.getFossilizedItems().keySet();
	...
	{
		if (itemStack != null && DEDatabaseRegistry.FOLIAGE_NUTRIENTS_LIST != null && !DEDatabaseRegistry.FOLIAGE_NUTRIENTS_LIST.isEmpty())
	...
	public static boolean canPlaceRequiredItem(ItemStack itemStack)
	{
		if (itemStack != null)
		{
			for (Foliage foliage : DEDatabaseRegistry.LIST_ALL_FOLIAGE.values())
			{
				ItemStack requiredItemStack = foliage.getRequiredItem();
				if (requiredItemStack != null && itemStack.getItem().equals(requiredItemStack.getItem()))
					return true;
			}
		}
		return false;
		...
		if (!playerIn.worldObj.isRemote)
		{
			...
			if (slot != null && slot.getHasStack())
			{
				...
				else if (index >= this.research_station.getSizeInventory())
				{
					...
					{
						if (ContainerBotanicalResearchStation.canPlaceFossil(stackInSlot))
						{
					...
					{
						if (ContainerBotanicalResearchStation.canPlaceNutrient(stackInSlot))
						{
							if (!mergeItemStack(stackInSlot, 1, 2, false))
								return null;
						}
						else if (ContainerBotanicalResearchStation.canPlaceRequiredItem(stackInSlot))
						{
