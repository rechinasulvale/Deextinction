...
public class WorldGenMinableMultipleStates extends WorldGenerator
{
	private IBlockState target;
	private final List<IBlockState> listOreBlock;
	private final int numberOfStates;
	private final int numberOfBlocks;
	private final Predicate blockToReplace;

	public WorldGenMinableMultipleStates(int numberOfBlocks, Predicate blockToReplace, IBlockState... listOreBlock)
	{
	...
	public WorldGenMinableMultipleStates(int numberOfBlocks, Predicate blockToReplace, List<IBlockState> listOreBlock)
	{
						...
						if (d12 * d12 + d13 * d13 < 1.0D)
						{
							for (int j2 = l; j2 <= k1; ++j2)
							{
								...
								if (d12 * d12 + d13 * d13 + d14 * d14 < 1.0D)
								{
									...
									if (worldIn.getBlockState(blockPos).getBlock().isReplaceableOreGen(worldIn, blockPos, this.blockToReplace))
									{
