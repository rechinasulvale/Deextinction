...
public class EntityDeinonychus extends EntityDeExtinctedPack
{
	...
	static
	{
		EntityDeinonychus.foodList.add(Items.beef);
		EntityDeinonychus.foodList.add(Items.porkchop);
		EntityDeinonychus.foodList.add(Items.chicken);
		EntityDeinonychus.foodList.add(Items.mutton);
		EntityDeinonychus.foodList.add(Items.fish);
		EntityDeinonychus.foodList.add(Items.cooked_beef);
		EntityDeinonychus.foodList.add(Items.cooked_porkchop);
		EntityDeinonychus.foodList.add(Items.cooked_chicken);
		EntityDeinonychus.foodList.add(Items.cooked_mutton);
		EntityDeinonychus.foodList.add(Items.cooked_fish);
		EntityDeinonychus.foodList.add(Items.bone);
	}

	public EntityDeinonychus(World worldIn)
	{
		super(worldIn);
	...
	{
		return DEDatabaseRegistry.LIST_DEEXTINCT_CREATURES.get(Deinonychus.NAME);
	}
	...
	protected void dropFewItems(boolean recentlyHit, int looting)
	{
		if (this.isIndependent())
		{
			this.dropItemWithOffset(DEItemRegistry.deinonychus_claw, this.rand.nextInt(2), 0.5F * this.height);
			if (this.isBurning())
				this.dropItemWithOffset(DEItemRegistry.deinonychus_cooked, this.rand.nextInt(2), 0.5F * this.height);
			else
				this.dropItemWithOffset(DEItemRegistry.deinonychus_raw, this.rand.nextInt(2), 0.5F * this.height);
		}
	}
}
