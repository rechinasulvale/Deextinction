...
public class EntityDinornis extends EntityDeExtinctedProtective
{
	...
	static
	{
		EntityDinornis.foodList.add(Items.apple);
		EntityDinornis.foodList.add(Items.melon);
		EntityDinornis.foodList.add(Items.melon_seeds);
		EntityDinornis.foodList.add(Items.bread);
		EntityDinornis.foodList.add(Items.wheat);
		EntityDinornis.foodList.add(Items.wheat_seeds);
	}

	public EntityDinornis(World world)
	{
		super(world);
	...
	{
		return DEDatabaseRegistry.LIST_DEEXTINCT_CREATURES.get(Dinornis.NAME);
	}
	...
	protected void dropFewItems(boolean recentlyHit, int looting)
	{
		...
		{
			this.dropItemWithOffset(DEItemRegistry.dinornis_feather, this.rand.nextInt(4), 0.8F * this.height);
			if (this.isBurning())
				this.dropItemWithOffset(DEItemRegistry.dinornis_cooked, this.rand.nextInt(3), 0.8F * this.height);
			else
				this.dropItemWithOffset(DEItemRegistry.dinornis_raw, this.rand.nextInt(3), 0.8F * this.height);
		}
	}
}
