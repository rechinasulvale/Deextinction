...
public class EntityUtahraptor extends EntityDeExtinctedPack
{
	...
	static
	{
		EntityUtahraptor.foodList.add(Items.beef);
		EntityUtahraptor.foodList.add(Items.porkchop);
		EntityUtahraptor.foodList.add(Items.chicken);
		EntityUtahraptor.foodList.add(Items.mutton);
		EntityUtahraptor.foodList.add(Items.fish);
		EntityUtahraptor.foodList.add(Items.cooked_beef);
		EntityUtahraptor.foodList.add(Items.cooked_porkchop);
		EntityUtahraptor.foodList.add(Items.cooked_chicken);
		EntityUtahraptor.foodList.add(Items.cooked_mutton);
		EntityUtahraptor.foodList.add(Items.cooked_fish);
		EntityUtahraptor.foodList.add(Items.bone);
	}

	public EntityUtahraptor(World worldIn)
	{
		super(worldIn);
	...
	{
		return DEDatabaseRegistry.LIST_DEEXTINCT_CREATURES.get(Utahraptor.NAME);
	}
	...
	{
		return DeExtinction.prependModID("utahraptor_ostrommaysorum");
	}

	@Override
	protected float getSoundPitch()
	{
		return this.isChild() ? this.rand.nextFloat() * 0.3F + 1.5F : this.rand.nextFloat() * 0.2F + 0.8F;
	}
	...
	protected void dropFewItems(boolean recentlyHit, int looting)
	{
		if (this.isIndependent())
		{
			this.dropItemWithOffset(DEItemRegistry.utahraptor_claw, this.rand.nextInt(2), 0.5F * this.height);
			if (this.isBurning())
				this.dropItemWithOffset(DEItemRegistry.utahraptor_cooked, this.rand.nextInt(2), 0.5F * this.height);
			else
				this.dropItemWithOffset(DEItemRegistry.utahraptor_raw, this.rand.nextInt(2), 0.5F * this.height);
		}
	}
}
